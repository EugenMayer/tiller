module Tiller
  def self.render(template, options={})
    ...
    if options.has_key?(:namespace)
      ...
      return ERB.new(template, trim_mode: '-').result(ns.instance_eval { b })
    end
    ...
    if options.has_key?(:direct_render)
      content = template
      return ERB.new(content, trim_mode: '-').result(ns.instance_eval { binding })
    end

    if Tiller::templates.key?(template)
      content = Tiller::templates[template]
      ERB.new(content, trim_mode: '-').result(ns.instance_eval { binding })
    else
